...
| 파일 | 요지 |
| --- | --- |
| `eslint.config.ts` | typescript-eslint(flat 네이티브, config-next 미사용) + import-x + FSD boundaries(`pageViews`) + oxlint |
...
| `postcss.config.mjs` | Tailwind v4 PostCSS 플러그인(`--tailwind` 없이 만든 앱 대비) |
...
> Vue와 Next는 실전 검증됐고(Next는 실서비스 MVP 부트스트랩에서 eslint·pageViews·vitest 구성이
> 검증·조정됨), React 템플릿은 표준 구성으로 번들했다 — 앱 생성기(`create-vite`/`create-next-app`)
> 기본과 병합해 쓰며, 버전은 각 `INSTALL.md`를 따른다.
