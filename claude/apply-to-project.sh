...
#   sh .fe-guide/claude/apply-to-project.sh <vue|react> [hub경로]
#   예)  sh .fe-guide/claude/apply-to-project.sh vue
#        sh .fe-guide/claude/apply-to-project.sh react vendor/guide
#
...
case "$FW" in
  vue | react) ;;
  *)
    echo "사용법: sh $HUB/claude/apply-to-project.sh <vue|react> [hub경로]"
    echo "  프레임워크(vue 또는 react)를 첫 인자로 지정하세요."
    exit 1
    ;;
esac

if [ ! -d "$HUB/claude/skills" ]; then
  echo "error: '$HUB/claude/skills' 를 찾을 수 없습니다."
  echo "  먼저 이 허브를 submodule로 추가하세요:  git submodule add git@github.com:parksubeom/bumpist-fe-guide.git $HUB"
  exit 1
fi

# 1) 런타임 스킬 심볼릭 링크 (.claude/skills -> ../$HUB/claude/skills)
mkdir -p .claude
if [ -L .claude/skills ] || [ -e .claude/skills ]; then
  echo "note: .claude/skills 가 이미 있습니다 — 건너뜁니다(필요하면 직접 확인)."
else
  ln -s "../$HUB/claude/skills" .claude/skills
  echo "created: .claude/skills -> ../$HUB/claude/skills"
fi
...
echo "----------------------------------------"
for f in "$HUB"/claude/rules/*.md; do
  [ -e "$f" ] || continue
  echo "@$f"
done
for f in "$HUB"/claude/rules/"$FW"/*.md; do
  [ -e "$f" ] || continue
  echo "@$f"
done
echo "----------------------------------------"
