...
2. **규칙을 안 외워도 Claude가 지킨다** — 규칙 13개 + 프레임워크별 규칙 + 스킬 14개가 `.claude/`로 배선된다.
...
그래서 제가 지키는 규칙을 전부 문서로 박제하고, 그 규칙을 실제 작업 절차로 옮기는 Claude Code 스킬을 만들어서 한 패키지로 묶었습니다. 그게 Bumpist Code입니다. npx bumpist-code@latest init 한 줄이면 프레임워크(Vue·React·Next.js)를 자동 감지해서 규칙과 스킬이 프로젝트의 .claude/로 복사됩니다. 이제 규칙을 외울 필요가 없습니다 — Claude가 작업 전에 규칙을 읽고 지키니까요.
...
- 규칙(사람+Claude가 같이 읽는 문서 13개 + 프레임워크별)과
  스킬(규칙을 작업 절차로 옮긴 것 14개)의 역할 분리
...
> 1트윗 280자 제한 고려해 짧게. 한국어 기준 여유 있음. 코드 블록 없이 붙여넣기.
...
```
Bumpist Code — 프론트엔드 공통 규칙 13개 + 프레임워크별(Vue·React·Next.js) 규칙 + Claude Code 스킬 14개.

설치는 이 한 줄이 전부입니다:

npx bumpist-code@latest init

프레임워크를 자동 감지해서 .claude/에 배선됩니다.
```
...
1. FSD pages 레이어 ↔ Next의 src/pages 예약 충돌 → pageViews로 개명
...
제가 지키는 규칙(공통 13개 + Vue/React/Next.js별)과 Claude Code 스킬 14개를
npx bumpist-code@latest init 한 줄로 프로젝트에 배선해 주는 CLI로 만들었습니다.
...
- [ ] re-pin 저장소 공개 상태·README 정돈 확인 (검증 클레임의 근거 링크이므로)
...
- **긱뉴스**: 자기 프로젝트 제출은 허용되지만 톤이 광고면 역효과. §1처럼 문제→접근→검증 순서 유지. 댓글에서 기술 질문(FSD, CLAUDE.md @import 원리 등)이 나오면 성실히 답하는 것이 최고의 홍보.
...
- **전 채널 공통**: 사용자 수·별점·다운로드 수는 언급하지 않는다(초기라 근거 없음). "실전 검증"의 근거는 항상 re-pin 링크로 구체화한다.
