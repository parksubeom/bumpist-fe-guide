...
## 프레임워크는 프로젝트당 하나 — Vue · React · Next.js

프로젝트를 시작할 때 **Vue · React · Next.js 중 하나**를 고른다(`setup-fe-project`가 먼저 묻는다). 고른
프레임워크의 규칙을 **`rules/vue/`** · **`rules/react/`** · **`rules/next/`**에서 함께 읽는다. 아래 스택
표의 "공통"은 셋 다 동일하고, "프레임워크별"만 갈린다.

## 스택 (고정)

**공통 (Vue·React·Next.js 동일)**

- **TypeScript strict** · 빌드는 **Vite**(Vue·React) / **Next 자체 빌드**(Next.js, App Router)
...
|                 | Vue (`rules/vue/`)     | React (`rules/react/`)   | Next.js (`rules/next/`, App Router) |
| --------------- | ---------------------- | ------------------------ | ----------------------------------- |
| 컴포넌트        | Vue 3 `<script setup>` | 함수 컴포넌트 + 훅 (TSX) | RSC 기본 + `'use client'` 잎        |
| 라우팅          | Vue Router             | React Router             | App Router (파일 기반)              |
| 클라이언트 상태 | Pinia                  | Zustand                  | Zustand                             |
| 서버 상태       | Vue Query              | React Query              | RSC fetch + Server Action + React Query |
| i18n            | vue-i18n               | react-i18next            | next-intl                           |
| 컴포넌트 테스트 | `@vue/test-utils`      | React Testing Library    | React Testing Library               |
...
| `pnpm type-check`             | vue-tsc(Vue) / tsc(React·Next) |
