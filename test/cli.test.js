...
// 격리된 임시 프로젝트에서 CLI를 돌리고 결과를 돌려준다. 폴더는 t.after 로 정리하므로
// 테스트 본문에서 반환된 dir 을 계속 읽을 수 있다. (finally 즉시 삭제하면 assert 전에 사라진다)
// setup은 프로젝트 폴더를 준비하는 콜백(예: package.json/CLAUDE.md 심기).
function runInTemp(t, cliArgs, setup) {
...
for (const fw of ['vue', 'react', 'next']) {
  test(`init ${fw}: 스킬·규칙·버전·CLAUDE.md를 배선한다`, (t) => {
    const { dir, status } = runInTemp(t, ['init', fw])
...
test('프레임워크 인자 없으면 package.json에서 자동 감지한다', (t) => {
  const { dir, stdout, status } = runInTemp(t, [], (d) => {
    fs.writeFileSync(
...
test('next 는 react보다 우선 감지된다', (t) => {
  const { dir } = runInTemp(t, [], (d) => {
    fs.writeFileSync(
      path.join(d, 'package.json'),
...
test('재실행해도 크래시 없이 idempotent 하다 (Node 24 재귀 fs 회귀 방지)', (t) => {
...
test('잘못된 프레임워크는 exit 1로 실패한다', (t) => {
  const { status, stdout } = runInTemp(t, ['init', 'svelte'])
  assert.equal(status, 1)
  assert.match(stdout, /vue·react·next/)
})

test('감지 실패 시(빈 폴더, 인자 없음) exit 1', (t) => {
  const { status, stdout } = runInTemp(t, [])
  assert.equal(status, 1)
  assert.match(stdout, /감지하지 못/)
})

test('기존 CLAUDE.md 는 덮어쓰지 않고 추가할 줄을 안내한다', (t) => {
  const original = '# 내 프로젝트\n\n중요한 기존 내용\n'
  const { dir, stdout } = runInTemp(t, ['init', 'vue'], (d) => {
...
test('-h / --help 는 사용법을 출력하고 exit 0', (t) => {
  const { status, stdout } = runInTemp(t, ['-h'])
  assert.equal(status, 0)
