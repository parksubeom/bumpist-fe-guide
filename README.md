...
sh .fe-guide/apply-to-project.sh vue        # 또는 react
...
git submodule add git@github.com:parksubeom/bumpist-fe-guide.git .fe-guide
sh .fe-guide/apply-to-project.sh vue        # 또는 react → @import 줄을 CLAUDE.md에
```
...
- **새 머신에서 clone**: `git clone --recurse-submodules <내프로젝트>` (또는 clone 후 `git submodule update --init`).
...
**태그를 checkout 하면 그 버전에 고정**된다 — 별도 버전 파일 없이 submodule 포인터가 곧 버전 기록이다.
프로젝트마다 다른 태그로 고정해도 서로 독립이다.
...
git add .fe-guide .claude/skills CLAUDE.md && git commit -m "chore: bump guide v0.1.0 → v0.3.0"
```
...
이 저장소가 규칙·스킬의 단일 출처이고, 다른 프로젝트에서 git submodule로 참조한다(적용법은
[install.md](./install.md)). 파일은 여기 한곳에서만 관리하고, 소비 프로젝트가 읽는 위치와는
링크·import로 연결한다.

![구조 연결: 이 저장소(rules·skills)가 단일 출처이고, CLAUDE.md가 rules를 @import, 소비 프로젝트의 .claude/skills 심볼릭 링크가 저장소의 skills를 가리키며 Claude Code 런타임이 규칙 적용·스킬 로드](./assets/structure.svg)

정리하면 이렇다. 규칙은 `CLAUDE.md`가 @import로 불러와 런타임에 적용된다. 스킬은 소비 프로젝트의
`.claude/skills`에서 로드되는데, 그 경로가 이 저장소의 `skills/`를 가리키는 심볼릭 링크다.
그래서 파일은 이 저장소에서만 관리하면 된다. 적용 방법은 [install.md](./install.md)에 있다.
