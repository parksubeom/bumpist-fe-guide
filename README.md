...
> **Bump the rules. Ship consistent frontends.** — 버전을 bump하듯, 규칙도 bump한다.

이름 그대로다 — semver에서 버전을 `bump`하듯, 규칙도 bump한다. 좋은 규칙은 한 번 정하고 끝나는
문서가 아니라, 실전에서 깨질 때마다 고쳐서 버전을 올리는 살아 있는 표준이라고 믿는다. 바이브 코딩으로
결과물은 빨리 나오는데 프로젝트마다 구조가 다르고, 보안이 허술하고, 퀄리티가 복불복인 게 싫어서 만들었다.
그래서 규칙을 "읽어야 하는 문서"로 두지 않고 Claude 스킬로 **실행**되게 배선했다 — 규칙은 문서가 아니라
작업 절차로 돈다. 1인 메인테이너가 실제 프로젝트에 직접 쓰면서 다듬는 중이고, 실전에서 깨진 것은
규칙으로 환류돼 다음 bump가 된다. ([실전 사례](#실전-사례-showcase)가 그 기록이다.)
...
- ⭐ **Star** — 이 가이드가 프로젝트 하나라도 덜 무너지게 했다면, Star 하나가 다음 bump의 동력이 된다.
