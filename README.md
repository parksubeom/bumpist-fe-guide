...
이 repo를 프로젝트로 **복사**해서 쓴다(서브모듈 X). 허브를 잠깐 받아 AI가 쓰는 문서만 `.claude/`로
복사하고, 임시 폴더는 지운다. 복사본이라 **git 몰라도 되고, 서브모듈·윈도우 함정이 없다.** 프레임워크
(**Vue**/**React**)를 인자로 준다. (자세한 절차는 [install.md](./install.md))
...
sh .bumpist/apply-to-project.sh vue .bumpist    # 또는 react — .claude/로 복사 + 버전 기록
rm -rf .bumpist                                  # 임시 폴더 삭제
# → 출력된 @import 줄(@.claude/rules/...)을 루트 CLAUDE.md에 붙여넣기
git add .claude docs/ai CLAUDE.md && git commit -m "chore: adopt bumpist-code guide"
```

확인: `ls .claude/skills/`에 스킬이 보이면 Claude가 규칙·스킬을 인지한 것.

> 최신을 받으려면 `--branch v0.1.0`을 뺀다. `degit`을 선호하면:
> `npx degit parksubeom/bumpist-fe-guide#v0.1.0 .bumpist` → `sh .bumpist/apply-to-project.sh vue .bumpist v0.1.0`.

### 시나리오 B — 새 프로젝트를 처음부터 (빈 폴더)

빈 폴더에서 위 A의 복사를 먼저 한 뒤, Claude Code에서 **"새 프로젝트 셋업해줘"** →
...
- **팀원 clone**: 복사본이라 그냥 `git clone <내프로젝트>` 면 된다(서브모듈 초기화·심볼릭 링크 없음).
- **표준 최신화**: 위 복사(A)를 원하는 버전으로 **다시 실행**하면 `.claude/{skills,rules}`·`docs/ai`를
  덮어쓴다. (**자동 최신화는 안 된다** — 이 재복사가 갱신 방법이다.)

## 버전 관리 (git 태그)

허브는 릴리스마다 **git 태그**(`v1.2.0`)로 버전을 매긴다. 복사 방식이라 **원하는 버전을 골라** 받고,
받는 순간 그 버전이 프로젝트에 얼어붙는다(다시 복사하기 전엔 안 바뀜). 프로젝트마다 다른 버전을 써도 독립이다.

**버전 번호 (semver)**

- `MAJOR` (v2.0.0) — 규칙 삭제·이름/경로 변경 등 **깨지는 변경**
- `MINOR` (v1.3.0) — 새 규칙·스킬 추가, 규칙 보강
- `PATCH` (v1.2.4) — 문구·오타·작은 수정

**특정 버전으로 받기** — clone에 `--branch <태그>`(생략하면 최신), degit이면 `#태그`:

```sh
git clone --depth 1 --branch v0.1.0 https://github.com/parksubeom/bumpist-fe-guide.git .bumpist
```

받은 버전은 `.claude/.guide-version`에 기록된다("이 프로젝트는 어느 표준 버전인지"가 git에 남음).

**지금 몇 버전인지** — `cat .claude/.guide-version`

**버전 올리기/내리기** — 원하는 태그로 **다시 복사**(위 시나리오 A를 `--branch v0.3.0`으로 재실행)하면 된다.

**옛 버전 패치** — 특정 옛 계열만 고쳐야 하면 허브에서 그 태그로 유지보수 브랜치를 파생해 패치 태그
(예: `v0.1.1`)를 낸다.
